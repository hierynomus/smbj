...
public class DFSPathResolver implements PathResolver {
    ...
    private ReferralResult getReferral(DfsRequestType type, Share share, DFSPath path) throws TransportException, Buffer.BufferException, DFSException {
    ...
    private ReferralResult handleReferralResponse(DfsRequestType type, SMB2IoctlResponse response, DFSPath originalPath) throws Buffer.BufferException, DFSException {
    ...
    private void handleRootOrLinkReferralResponse(ReferralResult result, SMB2GetDFSReferralResponse response) throws DFSException {
        if (response.getReferralEntries().isEmpty()) {
            throw new DFSException(NtStatus.STATUS_OBJECT_PATH_NOT_FOUND);
        }
