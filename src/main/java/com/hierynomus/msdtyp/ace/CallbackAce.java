...
public class CallbackAce extends Ace {
    private long accessMask;
    private SID sid;
    private byte[] applicationData;

    CallbackAce(AceHeader header, long accessMask, SID sid, byte[] applicationData) {
        super(header);
    ...
    static CallbackAce read(AceHeader header, SMBBuffer buffer, int aceStartPos) throws Buffer.BufferException {
        ...
        return new CallbackAce(header, accessMask, sid, applicationData);
    }

    @Override
    public String toString() {
        return String.format(
            "CallbackAce{type=%s, flags=%s, access=0x%x, sid=%s, data=%s}",
