...
public class ObjectAce extends Ace {
    long accessMask;
    SID sid;
    UUID objectType;
    UUID inheritedObjectType;

    ObjectAce(AceHeader header) {
        super(header);
    }

    ObjectAce(AceHeader header, long accessMask, UUID objectType, UUID inheritedObjectType, SID sid) {
        super(header);
    ...
    void writeBody(SMBBuffer buffer) {
        ...
        EnumSet<AceObjectFlag> flags = EnumSet.noneOf(AceObjectFlag.class);
        if (objectType != null) {
            flags.add(AceObjectFlag.ACE_OBJECT_TYPE_PRESENT);
        }
        if (inheritedObjectType != null) {
            flags.add(AceObjectFlag.ACE_INHERITED_OBJECT_TYPE_PRESENT);
        }
    ...
    void readBody(SMBBuffer buffer, int aceStartPos) throws Buffer.BufferException {
        ...
        Set<AceObjectFlag> flags = toEnumSet(buffer.readUInt32(), AceObjectFlag.class);

        objectType = null;
        if (flags.contains(AceObjectFlag.ACE_OBJECT_TYPE_PRESENT)) {
        ...
        if (flags.contains(AceObjectFlag.ACE_INHERITED_OBJECT_TYPE_PRESENT)) {
    ...
    static ObjectAce read(AceHeader header, SMBBuffer buffer, int aceStartPos) throws Buffer.BufferException {
        ObjectAce ace = new ObjectAce(header);
    ...
    public String toString() {
        return String.format(
            "ObjectAce{type=%s, flags=%s, access=0x%x, objectType=%s, inheritedObjectType=%s, sid=%s}",
