...
public class ObjectCallbackAce extends ObjectAce {
    private byte[] applicationData;

    private ObjectCallbackAce(AceHeader header) {
        super(header);
    }

    ObjectCallbackAce(AceHeader header, long accessMask, UUID objectType, UUID inheritedObjectType, SID sid, byte[] applicationData) {
    ...
    static ObjectCallbackAce read(AceHeader header, SMBBuffer buffer, int aceStartPos) throws Buffer.BufferException {
        ObjectCallbackAce ace = new ObjectCallbackAce(header);
    ...
    public String toString() {
        return String.format(
            "ObjectAce{type=%s, flags=%s, access=0x%x, objectType=%s, inheritedObjectType=%s, sid=%s, data=%s}",
